...
class TestTeacher:
    ...
    def test_conduct_lesson_success(self, teacher, awake_child, playing_child):
        ...
        # Создаем игру с материалами и is_educational=True
        game = EducationalGame(
            "Рисование", 
            "Учимся рисовать", 
            3, 6, 
            ["Краски", "Кисточки"], 
            is_educational=True  # ВАЖНО!
        )
        
        # Создаем словарь материалов (ОБЪЕКТЫ)
        materials = {
        ...
        # Проверяем результат
        assert result["success"] is True
        assert "провел(а) учебное занятие 'Рисование'" in result["message"]
    ...
    def test_conduct_lesson_missing_materials(self, teacher, awake_child):
        ...
        materials = {
            "Краски": TeachingMaterial("Краски", 10),
            "Кисточки": TeachingMaterial("Кисточки", 0)  # закончились
    ...
    def test_conduct_lesson_filter_invalid_children(self, teacher, awake_child):
        ...
        assert result["success"] is True
        assert "провел(а) учебное занятие" in result["message"]
    ...
    def test_conduct_lesson_game_without_materials(self, teacher, awake_child):
        ...
        assert result["success"] is True
        assert "провел(а) учебное занятие 'Лото'" in result["message"]
    ...
    def test_conduct_lesson_material_runs_out(self, teacher):
        """Тест ситуации, когда материал заканчивается"""
        game = EducationalGame(
            "Кубики", 
            "Строим башню", 
            3, 6, 
        ...
        materials = {"Кубики": TeachingMaterial("Кубики", 1)}
        
        children = [Child("Аня", 4, initial_state="awake")]
        ...
        # Второе занятие с тем же материалом - не должно пройти
        children2 = [Child("Боря", 4, initial_state="awake")]
        result2 = teacher.conduct_lesson(children2, game, materials)
...
class TestEducationalGame:
    ...
    def test_create_game_valid(self):
        ...
        assert game.is_educational is False
    
    def test_create_game_educational(self):
        """Тест создания учебной игры"""
        game = EducationalGame("Рисование", "Учимся рисовать", 3, 6, 
                              ["Краски"], is_educational=True)
        assert game.is_educational is True
        assert game.required_materials == ["Краски"]
    ...
    def test_check_materials_all_present(self):
        """Тест проверки материалов - все есть"""
        game = EducationalGame("Рисование", "Описание", 3, 6, 
                              ["Краски", "Кисточки"])
        materials = {
            "Краски": TeachingMaterial("Краски", 10),
            "Кисточки": TeachingMaterial("Кисточки", 5),
        ...
        missing = game.check_materials(materials)
        assert missing == []
    
    def test_check_materials_missing(self):
        """Тест проверки материалов - некоторых нет"""
        game = EducationalGame("Рисование", "Описание", 3, 6,
                              ["Краски", "Кисточки", "Фломастеры"])
        materials = {
            "Краски": TeachingMaterial("Краски", 10),
        ...
        missing = game.check_materials(materials)
        assert "Кисточки" in missing
        assert "Фломастеры" in missing
        assert len(missing) == 2
    
    def test_check_materials_zero_quantity(self):
        """Тест проверки материалов - закончились"""
        game = EducationalGame("Рисование", "Описание", 3, 6,
                              ["Краски", "Кисточки"])
        materials = {
            "Краски": TeachingMaterial("Краски", 0),  # закончились
            "Кисточки": TeachingMaterial("Кисточки", 5)
        }
        
        missing = game.check_materials(materials)
        assert "Краски (закончились)" in missing
        assert len(missing) == 1
    
    def test_str_representation_game(self):
        """Тест строкового представления игры"""
        game = EducationalGame("Лото", "Классическое лото с картинками", 3, 6)
        expected = "🎮 ИГРА: Лото (для 3-6 лет): Классическое лото с картинками"
        assert str(game) == expected
    
    def test_str_representation_educational(self):
        """Тест строкового представления учебной игры"""
        game = EducationalGame("Рисование", "Учимся рисовать", 3, 6, 
                              ["Краски", "Кисточки"], is_educational=True)
        expected = " УЧЕБНОЕ: Рисование (для 3-6 лет): Учимся рисовать, нужно: Краски, Кисточки"
        assert str(game) == expected
    
    def test_str_representation_without_materials(self):
        """Тест строкового представления игры без материалов"""
        game = EducationalGame("Лото", "Классическое лото", 3, 6, is_educational=True)
        expected = " УЧЕБНОЕ: Лото (для 3-6 лет): Классическое лото"
        assert str(game) == expected
    
    def test_to_dict(self):
        """Тест сериализации в словарь"""
        game = EducationalGame("Мозаика", "Собираем узоры", 4, 7, 
                              ["Кубики", "Краски"], is_educational=True)
        data = game.to_dict()
        assert data == {
            "name": "Мозаика",
            ...
            "min_age": 4,
            "max_age": 7,
            "required_materials": ["Кубики", "Краски"],
            "is_educational": True
        }
    ...
    def test_from_dict(self):
        ...
        assert game.required_materials == ["Кубики"]
        assert game.is_educational is True
    
    def test_from_dict_without_optional(self):
        """Тест десериализации без опциональных полей"""
        data = {
            "name": "Пазлы",
            "description": "Складываем картинку",
            "min_age": 3,
        ...
        game = EducationalGame.from_dict(data)
        assert game.required_materials == []
        assert game.is_educational is False
...
class TestKindergarten:
    ...
    def test_start_game(self, kindergarten):
        ...
        assert result["success"] is True
        # ИСПРАВЛЕНО: проверяем, что состояние изменилось на "playing"
        assert child.state == "playing"
    
    def test_start_game_game_not_found(self, kindergarten):
        """Тест начала несуществующей игры"""
        child = kindergarten.add_child("Маша", 4)
        parent = kindergarten.add_parent("Ольга", child)
        ...
        with pytest.raises(GameNotFoundError):
            kindergarten.start_game("Маша", "Несуществующая")
