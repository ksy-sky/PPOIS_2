...
class Kindergarten:
    ...
    def add_game(self, name: str, description: str, min_age: int, max_age: int, 
                required_materials: List[str] = None, is_educational: bool = False) -> EducationalGame:
        game = EducationalGame(name, description, min_age, max_age, required_materials, is_educational)
        self.games.append(game)
        game_type = "УЧЕБНОЕ ЗАНЯТИЕ" if is_educational else " ИГРА"
        materials = f", материалы: {required_materials}" if required_materials else ""
        print(f"  {game_type}: {name} (для {min_age}-{max_age} лет){materials}")
        return game
