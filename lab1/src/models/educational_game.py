...
from src.utils.exceptions import GameAgeError

class EducationalGame:
    def __init__(self, name: str, description: str, min_age: int, max_age: int, 
                 required_materials: List[str] = None, is_educational: bool = False):
        if min_age > max_age:
        ...
        self.is_educational = is_educational  # True = учебное занятие, False = игра
    ...
    def __str__(self) -> str:
        game_type = " УЧЕБНОЕ" if self.is_educational else "🎮 ИГРА"
        materials = f", нужно: {', '.join(self.required_materials)}" if self.required_materials else ""
        return f"{game_type}: {self.name} (для {self.min_age}-{self.max_age} лет): {self.description}{materials}"
