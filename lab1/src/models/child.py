...
class Child:
    ...
    def __init__(self, name: str, age: int, initial_state: str = "left"):
        ...
        self.name = name
        self.age = age
        self._state = initial_state# __ Это сигнал программисту: «Не меняй эту переменную напрямую!». Меняй только через специальный метод.
        self._in_lesson = False  # Новый атрибут - участвует ли в учебном занятии 
    def make_hungry(self):
        if self.state in ["awake", "playing"]:
            self._state = "hungry"
            self._in_lesson = False  # при голоде сбрасываем учебный процесс
    ...
    def update_state(self, new_state: str) -> None:
        ...
        if new_state != "playing":  # если не играет, сбрасываем учебный процесс
